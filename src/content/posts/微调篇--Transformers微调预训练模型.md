...
\>>> tf\_dataset = model.prepare\_tf\_dataset(dataset\["train"], batch\_size=16, shuffle=True, tokenizer=tokenizer)
...
1. \>>> tokenized\_datasets = tokenized\_datasets.remove\_columns(\["text"])
...
2. \>>> tokenized\_datasets = tokenized\_datasets.rename\_column("label", "labels")



设置数据集的格式以返回 PyTorch 张量而不是`lists`：



3. \>>> tokenized\_datasets.set\_format("torch")
